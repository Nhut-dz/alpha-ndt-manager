  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-surface-800">Quản lý danh mục</h1>
          <p className="text-surface-500 text-sm mt-1">{categories.length} danh mục</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.length === 0 ? (
            <p className="text-surface-400 text-sm col-span-full text-center py-8">Chưa có danh mục nào</p>
          ) : categories.map((cat) => (
            <div key={cat.id} className="card hover:shadow-card-hover transition-shadow">
              <div className="flex items-start justify-between">
                ...
                <span className={cat.status === 1 ? 'badge-success' : 'badge-warning'}>
                  {cat.status === 1 ? 'Active' : 'Inactive'}
                </span>
              </div>
              <div className="flex items-center gap-2 mt-4 pt-3 border-t border-surface-100">
                <button onClick={() => openEdit(cat)} className="text-sm text-brand-500 hover:text-brand-600 flex items-center gap-1">
                  <Pencil size={14} /> Sửa
                </button>
                <button onClick={() => setDeleteId(cat.id)} className="text-sm text-red-500 hover:text-red-600 flex items-center gap-1 ml-auto">
                  <Trash2 size={14} /> Xóa
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-surface-800">
                {editItem.id ? 'Sửa danh mục' : 'Thêm danh mục'}
              </h3>
              <button onClick={() => setEditItem(null)} className="text-surface-400 hover:text-surface-600">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-surface-700 mb-1.5">Tên danh mục *</label>
                <input
                  type="text"
                  className="input-field"
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder="VD: Tin tức NDT"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-surface-700 mb-1.5">Mô tả</label>
                <textarea
                  className="input-field"
                  rows={3}
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setEditItem(null)} className="btn-secondary">Hủy</button>
      ...
      <ConfirmDialog
        open={deleteId !== null}
        title="Xóa danh mục"
        message="Bạn có chắc chắn muốn xóa danh mục này? Không thể xóa nếu có bài viết liên quan."

    ...
    <div className="card p-0 overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-surface-50 border-b border-surface-200">
              ...
              {actions && <th className="text-right px-4 py-3 font-medium text-surface-600">Thao tác</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-surface-100">
            {data.length === 0 ? (
              <tr><td colSpan={columns.length + (actions ? 1 : 0)} className="text-center py-8 text-surface-400">Không có dữ liệu</td></tr>
            ) : data.map((row, i) => (
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-3 border-t border-surface-200">
          <p className="text-sm text-surface-500">Trang {page} / {totalPages}</p>
